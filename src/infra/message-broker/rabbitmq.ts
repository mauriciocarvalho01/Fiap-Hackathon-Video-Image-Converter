...
import { MessageBroker } from '@/domain/contracts/gateways/message-broker';
import { logger } from '@/infra/helpers';

import * as amqp from 'amqplib';

export class RabbitMQ implements MessageBroker {
  public static connection: Promise<amqp.Connection> | null = null;
  private static channels: { [key: string]: amqp.Channel }[] = [];
  public static instance: RabbitMQ | null = null;
  ...
  static readonly getInstance = (
    ...
    if (RabbitMQ.connection === null) {
      RabbitMQ.connection = amqp.connect(messageBrokerOptions.host, {
        noDelay: true,
        clientProperties: {
  ...
  getChannel(channelName: string): MessageBroker.GenericType {
    if (RabbitMQ.channels.length === 0) {
      throw new MessageBrokerError('No available channels');
    }
    const channel = RabbitMQ.channels.find(
      (channel: MessageBroker.GenericType) => channel[channelName]
    );
